...
import com.lenovo.nlu.dialog.time.DateUtil;
...
public class TimeNormalizer implements Serializable {
	...
	public TimeNormalizer(final String path) {
		if (patterns == null) {
			try {
				...
				// System.out.println("re "+patterns.pattern());
	...
	public TimeUnit[] parse(final String target, final String timeBase) {
		this.target = target;
...
//		preHandling();
	 ...
	 * @return 时间单元数组
	 */
	public TimeUnit[] parse(final String target) {
		this.target = target;
...
//		target = stringPreHandlingModule.wordTranslator(target); // 替换 今明天随后几天
	...
	private TimeUnit[] TimeEx(String tar, final String timebase) {
		
		tar = stringPreHandlingModule.numberTranslator(tar);// 大写数字转化
//		 System.out.println("替换前："+tar);
		tar = stringPreHandlingModule.wordTranslator(tar); // 替换 今明天随后几天
//		System.out.println("替换后："+tar);
		Matcher match;
...
//				System.out.println("temp[rpointer] .:"+temp[rpointer] );
			}
			endline = match.end();
			rpointer++;
		}
...
//		System.out.println("temp:"+Time_Result[0]);
		////////////////////////////////////////////////////////////////////////
		/** 过滤无法识别的字段 */
	...
	public static TimeUnit[] filterTimeUnit(final TimeUnit[] timeUnit) {
		...
		final List<TimeUnit> list = new ArrayList<>();
	...
	public static void main(final String args[]) throws Exception {
		...
		final TimeNormalizer normalizer = new TimeNormalizer(classPath + "/TimeExp.m");

		normalizer.parse("今天天气");// 抽取时间：今天，十月一号到五号和十一月一号
		TimeUnit[] unit = normalizer.getTimeUnit();
		// System.out.println("unit[0].getTime():" + unit[0].getTime());
